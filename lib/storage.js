...
const addTag = (nick, tag) => {
  const stmt = `INSERT INTO Nicknames (nick, battletag) VALUES ('${nick}', '${tag}') ON CONFLICT (nick) DO UPDATE tag = '${tag}'`;
  console.log('trying to add tag: ', nick, tag, stmt);
  return db.none(stmt).catch(err =>  {
    console.error(err);
  })
};
...
const setChannel = channel => {
  ...
  return db.none(stmt).then(() => {
    getChannel().then(result => console.log(result));
  })
}
