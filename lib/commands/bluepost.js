...
// Test: |tydow bluepost https://us.battle.net/forums/en/overwatch/topic/20753516591#post-2
    ...
    fn: (argv, context) => {
        return new Promise((resolve, reject) => {
            get(argv.args.url).then(body => {
                ...
                postcontent = postcontent.replace(/<br>/g, '\n').replace(/<strong>(.*?)<\/strong>/g, '**$1**').replace(/<span class="underline">(.*?)<\/span>/g, '__$1__');
                let text = cheerio.load(postcontent).text();
                let date = moment('03/09/2017 12:54 AM', 'MM/DD/YYYY hh:mm a').format('MMMM DD YYYY');

                // console.log('author: ', author);
                // console.log('time: ', posttime);
                // console.log('content: ', text);
                //resolve(`blueposts are not implemented yet, original link: ${argv.args.url}`);
                resolve(`**${author} posted on ${date}: **\n${text}` );
            });
