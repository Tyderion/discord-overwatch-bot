...
exports.default = new index_js_1.Command({
    ...
    fn: function (argv, context) { return __awaiter(_this, void 0, void 0, function () {
        var nickname, region, platform, battletag, _a, info, allHeroes, playedHeroes, top5, embed_1, kd, heroDetails_1, err_1;
        return __generator(this, function (_b) {
            switch (_b.label) {
                ...
                case 3:
                    ...
                    logger.info("Loaded Profile and Hero Overview for " + battletag);
                    top5 = playedHeroes.filter(function (hero) { return hero.playtime !== '--'; }).slice(0, 5);
                    embed_1 = new discord_js_1.RichEmbed({
                    ...
                    embed_1.addField('Skill Rating', "**" + info.competitive.rank + "**", true);
                    embed_1.addField('Playtime', info.playtime.competitive, true);
                    embed_1.addField('Games', allHeroes.GamesPlayed + " (" + orZero(allHeroes.GamesWon) + ":" + orZero(allHeroes.GamesTied) + ":" + orZero(allHeroes.GamesLost) + ")", true);
                    embed_1.addField('Kills', allHeroes.FinalBlows, true);
                    embed_1.addField('Eliminations', allHeroes.Eliminations, true);
                    embed_1.addField('Deaths', allHeroes.Deaths, true);
                    embed_1.addField('Elims/Deaths', kd, true);
                    embed_1.addField('Medals', allHeroes.Medals + " (" + allHeroes.Medals_Gold + "/" + allHeroes.Medals_Silver + "/" + allHeroes.Medals_Bronze + ")", true);
                    embed_1.addField('Cards', allHeroes.Cards, true);
                    return [4 /*yield*/, owapi_1.getCompHeroDetails(battletag, region, platform, top5)];
                case 4:
                    heroDetails_1 = _b.sent();
                    logger.info("Loaded herodetails for " + battletag + ".");
                    Object.keys(heroDetails_1).forEach(function (name) {
                        var hero = heroDetails_1[name];
                        var time = hero.TimePlayed ? hero.TimePlayed : '0minutes';
                        var str = time.replace(/([0-9]+)([a-zA-Z]+)/, '$1 $2') + ": " + hero.WinPercentage + " wins";
                        embed_1.addField(owapi_1.convertNames.toDisplay(name), str);
                    });
                    context.responseConfig.embeds = [embed_1];
