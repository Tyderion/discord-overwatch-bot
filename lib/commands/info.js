    ...
    return new Promise((resolve, reject) => {
      return storage.get(nickname).then(result => {
        ...
        return Promise.all([
          ...
          const top5 = playedHeroes.slice(0, 5);
          // Promise.all(top5.map => )
          return ow.getCompHeroDetails(result.battletag, region, top5.map(h => h.name).join(',')).then(result => {
            
          process.env.NODE_TLS_REJECT_UNAUTHORIZED = "1";
            message += `\nMost Played Heroes (${top5.length})`
            const top5String = Object.keys(result).map(key => `\n${key} (${result[key].TimePlayed.replace(/([0-9]+)([a-zA-Z]+)/, '$1 $2')}) with **${result[key].WinPercentage}** wins`).join('');
            message += top5String;
            resolve('```' + message + '```');
            // resolve(' ');
          })
